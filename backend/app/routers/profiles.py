...
def list_profiles(
    ...
    if current_user:
        query = query.filter(UserProfile.user_id == current_user.id)
    else:
        query = query.filter(UserProfile.user_id.is_(None))
    records = query.order_by(UserProfile.created_at.desc()).limit(limit).all()
...
def latest_profile(
    ...
    if current_user:
        ...
        query = query.filter(UserProfile.user_id.is_(None))
    record = query.order_by(UserProfile.created_at.desc()).first()

...
try:
    raw_data = api.get_kpop_artists()
except Exception:
    raw_data = []

if not raw_data:
    st.error("K-pop data not available from backend. Please ensure the backend server is running.")
    st.stop()

df = pd.DataFrame(raw_data)

required_cols = [
    "artist_name", "artist_type", "company", "debut_year",
    "gender", "main_market", "secondary_market"
]

missing_cols = [col for col in required_cols if col not in df.columns]

if missing_cols:
    st.error(f"Missing columns in kpop data: {missing_cols}")
    st.stop()
...
US_GLOBAL_TARGET_GROUPS = (
    GLOBAL_MAINSTREAM + CURRENT_EXPANSION + EXPERIMENTAL_GLOBAL
)
...
def generate_industry_insight(data):
    ...
    group_ratio = round(
        len(data[data["artist_type"] == "group"]) / len(data) * 100, 1
    )

    company_concentration = round(
        data["company_group"].value_counts(normalize=True).max() * 100, 1
    )
...
def calculate_us_potential_by_company(data):
    ...
    result = []
    superstar_list = ["BTS", "BLACKPINK", "Stray Kids", "NewJeans", "aespa"]
    ...
    return pd.DataFrame(result).sort_values("us_potential_score", ascending=False)
...
def predict_next_global_hit(data):
    ...
    return pd.DataFrame(candidates).sort_values("global_hit_score", ascending=False)


def generate_us_strategy_text(group_name, us_score, english, performance, social_media, western_fit, fanbase):
    if us_score >= 8:
        level = "strong"
    elif us_score >= 6:
        level = "moderate"
...
st.title(tr("🎤 K-pop Industry Intelligence", "🎤 K-pop 产业智能分析"))

if st.button(tr("🏠 Back to Culture", "🏠 返回文化模块")):
...
st.caption(
    tr("A dedicated culture sub-module analysing K-pop as a global entertainment industry, with a focus on US market expansion.",
       "一个文化子模块，用数据分析 K-pop 作为全球娱乐产业的运作，并重点关注美国市场扩张。")
)

st.divider()

tabs = st.tabs([
    "🌐 Industry Overview", "🏢 Company Analysis",
    "📅 Generation & Market", "🇺🇸 US Market Prediction",
    "🌍 Global Hit Predictor", "🧠 Insight"
])

with tabs[0]:
    st.subheader(tr("🌐 K-pop Industry Overview", "🌐 K-pop 产业概览"))
...
""")
    ...
    st.subheader(tr("📋 Artist Dataset", "📋 艺人数据集"))
    st.dataframe(
...
with tabs[1]:
    st.subheader(tr("🏢 Company Analysis", "🏢 公司分析"))
    ...
    if company_df.empty:
        ...
        st.bar_chart(pd.crosstab(company_df["company_group"], company_df["artist_type"]))
        st.subheader("🌍 Market Focus by Company")
        st.bar_chart(pd.crosstab(company_df["company_group"], company_df["main_market"]))
        st.subheader("🚀 Expansion Category by Company")
        st.bar_chart(pd.crosstab(company_df["company_group"], company_df["expansion_category"]))
        st.info("""
...
with tabs[2]:
    st.subheader(tr("📅 Generation & Market Evolution", "📅 世代与市场演化"))

    col1, col2 = st.columns(2)
    with col1:
        st.subheader("📅 Debut Year Trend")
        st.line_chart(df["debut_year"].value_counts().sort_index())
    with col2:
    ...
    st.divider()
    st.subheader(tr("📜 Market Expansion Logic", "📜 市场扩张逻辑"))
...
with tabs[3]:
    st.subheader(tr("🇺🇸 US Market Prediction", "🇺🇸 美国市场预测"))
...
""")
    ...
    selected_group = st.selectbox("Choose a K-pop group", options=available_groups, index=default_index)
    ...
    us_score = (
        english * 0.2 + performance * 0.2 + social_media * 0.2 +
        western_fit * 0.2 + fanbase * 0.2
    )
    ...
    st.subheader(tr("📄 Rule-Based Strategy Report", "📄 规则型策略报告"))
    st.markdown(
        generate_us_strategy_text(selected_group, us_score, english, performance, social_media, western_fit, fanbase)
    )

    st.divider()

    st.subheader(tr("🤖 AI US Market Analysis", "🤖 AI 美国市场分析"))

    if st.button("Generate AI US Market Report"):
        with st.spinner("Analysing US market potential..."):
            try:
                report = api.generate_ai("kpop_us_analysis", {
                    "group_name": selected_group,
                    "us_score": round(us_score, 2),
                    "english": english,
                    "performance": performance,
                    "social_media": social_media,
                    "western_fit": western_fit,
                    "fanbase": fanbase,
                })

                st.markdown("""
                <div style="padding: 1.5rem; border-radius: 8px; background: #ffffff;
                 border: 1px solid #dbe3ef; box-shadow: 0 7px 20px rgba(15,23,42,0.05);">
...
with tabs[4]:
    st.subheader(tr("🌍 Next Global Hit Predictor", "🌍 下一位全球爆款预测"))
...
""")
    ...
    st.subheader(tr("🇺🇸 Company US Potential Score", "🇺🇸 公司美国市场潜力评分"))
...
with tabs[5]:
    st.subheader(tr("🧠 K-pop System Insight", "🧠 K-pop 系统洞察"))
    st.markdown(generate_industry_insight(df))

    st.divider()

    st.subheader(tr("📌 Project Positioning", "📌 项目定位"))
    st.success("""

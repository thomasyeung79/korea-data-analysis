...
st.title(tr("⚽ East Asian Football Pathway Analytics", "⚽ 东亚足球路径分析"))

st.caption(
    tr("A football analytics module comparing China, Japan, and Korea through Premier League and UEFA Champions League player pathways.",
       "通过英超和欧冠球员路径比较中国、日本与韩国的足球发展。")
)

if st.button(tr("🏠 Back to Sports", "🏠 返回体育模块")):
...
try:
    epl_data = api.get_epl_data()
except Exception:
    epl_data = []

try:
    ucl_data = api.get_ucl_data()
except Exception:
    ucl_data = []

if not epl_data:
    st.error("EPL data not available from backend.")
    st.stop()

epl_df = pd.DataFrame(epl_data)

epl_required_cols = ["player_name", "country", "player_type", "club", "competition_type"]
...
ucl_df = pd.DataFrame(ucl_data) if ucl_data else pd.DataFrame()
if not ucl_df.empty:
...
def show_epl_metrics(data):
    ...
    col5.metric("League Records", len(data[data["competition_type"] == "League"]))
...
def show_ucl_metrics(data):
    ...
    col3.metric("Countries", data["country"].nunique())
    col4.metric("Clubs", data["club"].nunique())
    col5.metric("Appeared Records", len(data[data["ucl_status"].str.lower() == "appeared"]))


def generate_system_insight(epl_data_df, ucl_data_df):
    epl_top_country = epl_data_df["country"].value_counts().idxmax()
    epl_players = epl_data_df["player_name"].nunique()
    epl_big6_players = epl_data_df[epl_data_df["big6"]]["player_name"].nunique()

    if not ucl_data_df.empty:
        ucl_top_country = ucl_data_df["country"].value_counts().idxmax()
        ucl_players = ucl_data_df["player_name"].nunique()
        appeared_players = ucl_data_df[ucl_data_df["ucl_status"].str.lower() == "appeared"]["player_name"].nunique()
    else:
        ucl_top_country = "N/A"
        ucl_players = 0
        appeared_players = 0
...
tabs = st.tabs([
    "🌏 EPL Overview", "🇰🇷 Korea Focus", "🏆 EPL Big 6",
    "⭐ UCL Analysis", "🔍 Player Explorer", "🔮 Next Korean Star", "🧠 Insight"
])

with tabs[0]:
    st.subheader(tr("🌏 East Asia EPL Overview", "🌏 东亚英超概览"))
...
""")
    ...
    st.subheader(tr("📋 Filtered EPL Dataset", "📋 筛选后的英超数据"))
    st.dataframe(
    ...
    with col_a:
        ...
        st.bar_chart(filtered_epl_df.drop_duplicates("player_name")["country"].value_counts())
    with col_b:
    ...
    with col_c:
        ...
        st.bar_chart(filtered_epl_df.drop_duplicates("player_name")["player_type"].value_counts())
    with col_d:
        st.subheader("🏟️ Competition Type")
        st.bar_chart(filtered_epl_df["competition_type"].value_counts())

with tabs[1]:
    st.subheader(tr("🇰🇷 Korea Focus: Elite Football Pathway", "🇰🇷 韩国重点：精英足球路径"))
...
""")
    ...
    st.markdown(f"## {tr('⚽ Korean EPL Pathway', '⚽ 韩国英超路径')}")
    ...
    if korea_epl_df.empty:
        ...
        col4.metric("Big 6 Players", korea_epl_df[korea_epl_df["big6"]]["player_name"].nunique())
    ...
    st.divider()
    st.markdown(f"## {tr('⭐ Korean UCL Pathway', '⭐ 韩国欧冠路径')}")

    if korea_ucl_df.empty:
        st.warning("No Korean UCL records available.")
    else:
        show_ucl_metrics(korea_ucl_df)
        st.dataframe(korea_ucl_df.sort_values(by=["player_name", "club"]), use_container_width=True)
    ...
    st.divider()
    st.subheader(tr("🧠 Korea Football System Insight", "🧠 韩国足球系统洞察"))
    st.success("""
...
with tabs[2]:
    st.subheader(tr("🏆 EPL Big 6 Analysis", "🏆 英超 Big 6 分析"))
...
""")
    ...
    big6_summary = pd.crosstab(epl_df["country"], epl_df["big6"])
...
with tabs[3]:
    st.subheader(tr("⭐ UEFA Champions League Analysis", "⭐ 欧冠分析"))
...
""")
        ...
        with col3:
            ucl_conf_options = sorted(ucl_df["confidence"].dropna().unique()) if "confidence" in ucl_df.columns else []
            if ucl_conf_options:
                selected_confidence = st.multiselect(
                    "Confidence",
                    options=ucl_conf_options,
                    default=ucl_conf_options
                )
            else:
                selected_confidence = []
        ...
        if selected_confidence and "confidence" in ucl_df.columns:
            filtered_ucl_df = filtered_ucl_df[filtered_ucl_df["confidence"].isin(selected_confidence)]
        ...
        with col_a:
            ...
            st.bar_chart(filtered_ucl_df.drop_duplicates("player_name")["country"].value_counts())
        with col_b:
        ...
        with col_d:
            if "confidence" in filtered_ucl_df.columns:
...
with tabs[4]:
    st.subheader(tr("🔍 Player Explorer", "🔍 球员探索器"))

    dataset_choice = st.radio("Choose Dataset", ["EPL", "UCL"], horizontal=True)

    if dataset_choice == "EPL":
        selected_country = st.selectbox("Choose Country", options=["All"] + sorted(epl_df["country"].unique()))
        explorer_df = epl_df.copy() if selected_country == "All" else epl_df[epl_df["country"] == selected_country]
        selected_player = st.selectbox("Select Player", options=sorted(explorer_df["player_name"].unique()))
...
**{clubs}**
Competition record type: **{competition_types}**
""")
        ...
        if ucl_df.empty:
            ...
            selected_country = st.selectbox("Choose Country", options=["All"] + sorted(ucl_df["country"].unique()))
            explorer_df = ucl_df.copy() if selected_country == "All" else ucl_df[ucl_df["country"] == selected_country]
            selected_player = st.selectbox("Select UCL Player", options=sorted(explorer_df["player_name"].unique()))
            player_df = explorer_df[explorer_df["player_name"] == selected_player]
...
with tabs[5]:
    st.subheader(tr("🔮 Next Korean Star Model", "🔮 下一位韩国球星模型"))
...
""")
    ...
    position = st.selectbox("Position", ["Forward", "Winger", "Midfielder", "Defender", "Goalkeeper"])
    ...
    park_score = work_rate * 0.3 + tactical_discipline * 0.3 + physicality * 0.2 + european_readiness * 0.2
    son_score = finishing * 0.3 + speed * 0.25 + marketability * 0.25 + european_readiness * 0.2
    ...
    if st.button("🤖 Generate AI Star Potential Analysis"):
        with st.spinner("Analysing player pathway..."):
            try:
                result = api.generate_ai("football_pathway", {
                    ...
                    "european_readiness": european_readiness,
                })

                st.markdown("""
                <div style="padding: 1.5rem; border-radius: 8px; background: #ffffff;
                 border: 1px solid #dbe3ef; box-shadow: 0 7px 20px rgba(15,23,42,0.05); margin-top: 1rem;">
...
with tabs[6]:
    st.subheader(tr("🧠 System Insight", "🧠 系统洞察"))
    st.markdown(generate_system_insight(epl_df, ucl_df))

    st.divider()
    st.subheader(tr("📌 Data Notes", "📌 数据说明"))
    st.write("""
